...
class RemoveRunningRecordMediator @Inject constructor(
    ...
    suspend fun removeWithRecordAdd(
        ...
        if (timerSyncBridge.enabled) {
            if (timeEnded != null) {
                timerSyncBridge.handler!!.reject("Synced timers currently stop at the current time. Backdated stops are not supported yet.")
                return
            }
            timerSyncBridge.handler!!.stop(runningRecord.id, runningRecord.timeStarted)
            return
        }
    ...
    suspend fun remove(
        ...
        if (timerSyncBridge.enabled) {
            timerSyncBridge.handler!!.reject("Deleting or changing a synced running timer is not supported yet. Use Stop timer.")
            return
        }
