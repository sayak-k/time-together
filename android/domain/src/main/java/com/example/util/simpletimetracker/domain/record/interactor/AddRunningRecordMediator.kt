...
class AddRunningRecordMediator @Inject constructor(
    ...
    private val timerSyncBridge: TimerSyncBridge = TimerSyncBridge(),
) {
    ...
    suspend fun tryStartTimer(
        ...
        if (timerSyncBridge.enabled) return timerSyncBridge.handler!!.start(typeId, "")
        // Already running
    ...
    suspend fun startTimer(
        ...
        if (timerSyncBridge.enabled) {
            if (timeStarted is StartTime.Timestamp || tags.isNotEmpty()) {
                timerSyncBridge.handler!!.reject("Synced timers currently support the current time and notes. Tags and backdated starts are not synced.")
                return
            }
            timerSyncBridge.handler!!.start(typeId, comment)
            return
        }
    ...
    suspend fun addAfterChange(
        ...
        if (timerSyncBridge.enabled) {
            timerSyncBridge.handler!!.reject("Editing a synced running timer is not supported yet. Stop it and start another activity.")
            return
        }
        addInternal(
            params = StartParams(
